import parseutils, strutils, tables, os, "../utils"
...
const opcs*: array[0 .. 0xFF, OpcodeRaw] = [
  ("NOP", 1, 4, 4, '-', '-', '-', '-'), ("LD BC,d16", 3, 12, 12, '-', '-', '-', '-'), ("LD (BC),A", 1, 8, 8, '-', '-', '-', '-'), ("INC BC", 1, 8, 8, '-', '-', '-', '-'), ("INC B", 1, 4, 4, 'Z', '0', 'H', '-'), ("DEC B", 1, 4, 4, 'Z', '1', 'H', '-'), ("LD B,d8", 2, 8, 8, '-', '-', '-', '-'), ("RLCA", 1, 4, 4, '-', '0', '0', 'C'), ("LD (a16),SP", 3, 20, 20, '-', '-', '-', '-'), ("ADD HL,BC", 1, 8, 8, '-', '0', 'H', 'C'), ("LD A,(BC)", 1, 8, 8, '-', '-', '-', '-'), ("DEC BC", 1, 8, 8, '-', '-', '-', '-'), ("INC C", 1, 4, 4, 'Z', '0', 'H', '-'), ("DEC C", 1, 4, 4, 'Z', '1', 'H', '-'), ("LD C,d8", 2, 8, 8, '-', '-', '-', '-'), ("RRCA", 1, 4, 4, '0', '0', '0', 'C'),
  ("STOP 0", 2, 4, 4, '-', '-', '-', '-'), ("LD DE,d16", 3, 12, 12, '-', '-', '-', '-'), ("LD (DE),A", 1, 8, 8, '-', '-', '-', '-'), ("INC DE", 1, 8, 8, '-', '-', '-', '-'), ("INC D", 1, 4, 4, 'Z', '0', 'H', '-'), ("DEC D", 1, 4, 4, 'Z', '1', 'H', '-'), ("LD D,d8", 2, 8, 8, '-', '-', '-', '-'), ("RLA", 1, 4, 4, '-', '0', '0', 'C'), ("JR r8", 2, 12, 12, '-', '-', '-', '-'), ("ADD HL,DE", 1, 8, 8, '-', '0', 'H', 'C'), ("LD A,(DE)", 1, 8, 8, '-', '-', '-', '-'), ("DEC DE", 1, 8, 8, '-', '-', '-', '-'), ("INC E", 1, 4, 4, 'Z', '0', 'H', '-'), ("DEC E", 1, 4, 4, 'Z', '1', 'H', '-'), ("LD E,d8", 2, 8, 8, '-', '-', '-', '-'), ("RRA", 1, 4, 4, '0', '0', '0', 'C'),
...
const prefixOpcs*: array[0 .. 0xFF, OpcodeRaw] = [
...
var operandTable = toTable[string, Operand]({
  ...
  "SP": createRegOperand("SP", RegSP, false),
  ...
  "(HL+)": Operand(name: "(HL+)", kind: Address, op: "HL", increment: true),
  "(HL-)": Operand(name: "(HL-)", kind: Address, op: "HL", decrement: true)
})

for i in 0 .. 7:
  operandTable[$i] = Operand(name: $i, kind: IntLit, num: i)

proc parseOperand(opc: OpcodeRaw, i: var int): string =
  var encoded = ""
  i.inc parseUntil(opc.mnemonic, encoded, {',', '\0'}, i)

  if encoded[0] == '(':
    var onlyRegister = ""
    discard parseUntil(encoded, onlyRegister, {')'}, 0)

    result = onlyRegister.toUpper() & ')'

  if opc.mnemonic[i] == ')': i.inc
  if opc.mnemonic[i] == ',': i.inc

  if result.isNil:
    result = encoded.toUpper()
...
when isMainModule:
  ...
  for a in 0 .. 0xFF:
    ...
    var operandOne = ""
    if opc.mnemonic[i] != '\0':
      operandOne = parseOperand(opc, i)
    var operandTwo = ""
    if opc.mnemonic[i] != '\0':
      operandTwo = parseOperand(opc, i)

    nopcs.add("Opcode(mnemonic: \"$1\", operandOne: \"$2\", operandTwo: \"$3\", len: $4, cycles: $5, idleCycles: $6, z: '$7', n: '$8', h: '$9', c: '$10', opcode: 0x$11),\n" %
        [opcType.toUpper(), operandOne.toUpper(), operandTwo.toUpper(),
         $opc.len, $opc.cycles, $opc.idleCycles, $opc.z, $opc.n, $opc.h, $opc.c,
         a.uint8.toHex()])

writeFile(getCurrentDir() / "opcodes.out", nopcs)
